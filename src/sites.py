...
class Kijiji(Site):
	...
	def _get_price(cls, item):
		...
		try:
			...
			raise ConvertPriceError(site=cls, text=item.text)
		return price
	@classmethod
	def _get_image(cls, item):
...
class MarketPlace(Site):
	...
	def _get_items(cls, driver):
		...
		items = WebDriverWait(driver, 4).until(lambda x: x.find_elements(By.XPATH, xpath))
		return items
	@classmethod
	def _get_link(cls, item):
		xpath = '//a[@href]'
		# print(item.get_attribute('outerHTML'))
		# link = item.find_element(By.XPATH, xpath)
